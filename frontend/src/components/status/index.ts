// Status Template Components
export {
	StatusTemplate,
	StatusCard,
	type StatusTemplateProps,
	type StatusCardProps,
} from './StatusTemplate'

// TanStack Router Integration Components
export { DefaultNotFound, type DefaultNotFoundProps } from './DefaultNotFound'
export { DefaultError, type DefaultErrorProps } from './DefaultError'
export { DefaultPending, type DefaultPendingProps } from './DefaultPending'
export { DefaultCatchBoundary, type DefaultCatchBoundaryProps } from './DefaultCatchBoundary'
